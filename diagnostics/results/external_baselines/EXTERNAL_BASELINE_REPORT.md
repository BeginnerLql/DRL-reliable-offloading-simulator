...
**Does PPO remain necessary given a reliability-safe action set?** In this 10-seed evaluation, Safe Min-Latency matched Masked PPO stochastic exactly on overall RSR (0.9727) and highest-tier RSR (0.8916), while producing higher mean reward (+4.2118 over PPO) and lower mean/P95 latency (by 0.1656 s / 0.4639 s); each paired difference favored Safe Min-Latency in all 10 seeds, with 95% paired bootstrap intervals excluding zero. PPO had lower pair-selection HHI in all 10 seeds, indicating a more even pair distribution, while the maximum-mean-queue interval overlapped zero. Thus this experiment does not show that PPO is necessary to achieve the tested reliability and reward/latency outcomes; Safe Min-Latency is stronger on those measured outcomes, while PPO retains a measurable diversity advantage. Max-Reliability attained the same RSR but substantially lower reward and higher latency. These are results for the tested workload and saved checkpoints, not a universal claim.
