...
class PairFailureCorrelationTests(unittest.TestCase):
    ...
    def _pair_probabilities(rho, beta=0.8, sample_count=40_000, seed=2026):
        ...
        independent_rates = base_rates[np.newaxis, :] * compute_hazard_multipliers(
            independent_z,
            beta,
        )
        return (
            compute_failure_probability_samples(spatial_rates, service_times),
            compute_failure_probability_samples(independent_rates, service_times),
        )

    def test_beta_zero_spatial_and_independent_joint_failure_match(self):
        spatial, independent = self._pair_probabilities(rho=0.8, beta=0.0)
        metrics = pair_failure_statistics(
            spatial[:, 0], spatial[:, 1], independent[:, 0], independent[:, 1]
        )
        self.assertAlmostEqual(metrics["joint_spatial"], metrics["joint_independent"], places=14)
        self.assertAlmostEqual(metrics["joint_failure_amplification"], 1.0, places=12)
        self.assertAlmostEqual(metrics["binary_failure_correlation"], 0.0, places=12)

    def test_zero_latent_correlation_gives_marginal_product(self):
        spatial, independent = self._pair_probabilities(rho=0.0, beta=0.8)
        metrics = pair_failure_statistics(
            spatial[:, 0], spatial[:, 1], independent[:, 0], independent[:, 1]
        )
    ...
    def test_positive_latent_correlation_increases_joint_failure(self):
        spatial, independent = self._pair_probabilities(rho=0.8, beta=0.8)
        metrics = pair_failure_statistics(
            spatial[:, 0], spatial[:, 1], independent[:, 0], independent[:, 1]
        )
        self.assertGreater(
    ...
    def test_equal_node_joint_failure_increases_with_rho(self):
        ...
        for rho in (0.0, 0.3, 0.8):
            spatial, independent = self._pair_probabilities(rho=rho, beta=0.8)
            metrics = pair_failure_statistics(
                spatial[:, 0], spatial[:, 1], independent[:, 0], independent[:, 1]
            )
    ...
    def test_spatial_and_independent_marginals_are_consistent(self):
        spatial, independent = self._pair_probabilities(rho=0.8, beta=0.8)
        differences = np.abs(spatial.mean(axis=0) - independent.mean(axis=0))
        self.assertLess(float(differences.max()), 0.01)
    ...
    def test_diagnostic_outputs_only_distinct_server_pairs(self):
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            for filename in (
                ...
                "pair_failure_correlation_summary.csv",
                "pair_failure_global_summary.csv",
            ):
